Param(
    [string]$SourceDir = $(Join-Path $PSScriptRoot "frontend\community-priorities-assets-for-export-src"),
    ...
    [string]$TargetDir = $(Join-Path $PSScriptRoot "frontend\dist\community-priorities-assets-for-export-map")
)
...
foreach ($file in $requiredSourceFiles) {
    ...
    if (!(Test-Path $path)) {
        Write-Error "Required Community Priorities and Assets for Export source file missing: $path"
        exit 1
    }
}
...
$optionalSourceFiles = @(
    "src\map-export-callouts.js",
    "src\export-callouts.css",
    "src\export-callout-layout.js"
)

foreach ($file in $optionalSourceFiles) {
    $path = Join-Path $SourceDir $file
    if (Test-Path $path) {
        Copy-Item $path (Join-Path $TargetDir $file) -Force
    }
}
...
Write-Host "Done. Community Priorities and Assets for Export map packaged into frontend/dist/community-priorities-assets-for-export-map."
